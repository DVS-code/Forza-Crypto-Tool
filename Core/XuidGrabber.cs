...
internal static class XuidGrabber
{
    ...
    public static async Task<ulong> GrabAsync(Action<string>? status = null)
    {
        ...
        foreach (var (name, pid) in candidates)
        {
            ...
            var xuidText = await FetchXuidAsync(xauth);
            if (!string.IsNullOrWhiteSpace(xuidText) && ulong.TryParse(xuidText, out var xuid))
            {
    ...
    private static async Task<string?> FetchXuidAsync(string xauth)
    {
        try
        {
